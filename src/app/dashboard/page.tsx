...
import { employers, payrollRuns, workers } from '@/server/db/schema';
...
async function getDashboardData() {
  ...
  const runs = await db
    .select()
    .from(payrollRuns)
    .where(eq(payrollRuns.employerId, employer.id))
  ...
  const totalDisbursed = runs
    ...
    .reduce((sum, r) => sum + BigInt(r.totalUsdc), 0n)
    .toString();

  return { employer, runs, workerCount: workerCount?.count ?? 0, totalDisbursed };
}
          ...
          {runs.filter((r) => !activeRun || r.id !== activeRun.id).length === 0 ? (
            <div className="bg-white rounded-xl border border-dashed border-gray-300 p-12 text-center">
              ...
              <p className="text-sm text-gray-400 mt-1">
                Upload a worker roster to get started
              </p>
