...
const pool = new Pool({
  connectionString:
    ...
    'postgresql://postgres:postgres@localhost:5432/stellar_agent_c',
});
...
async function seed() {
  ...
  const workerData = [
    ...
    {
      name: 'Linh Tran',
      stellarAddress: 'GBJCHHHWRP5MQJBQWGQHM2JBKRSKBBZHJ4MFZPJSYMIJBTLHXLHKDHY',
      phone: '+84987654321',
    ...
    {
      name: 'Priya Sharma',
      stellarAddress: 'GAKFD6Z3BQDCMHAKB7K4S4PPHFQHXHWXBH5SXBF6WLVHF5GQFHXKKWL',
      phone: '+601234567890',
      bankAccount: 'PBB-7654321098',
      status: 'registered' as const,
    },
    {
      name: 'Kevin Lim',
      stellarAddress: 'GCKRTVGWXDMXKGGCBVQCHB5PQZXKR3MQLHJLHLPZXHGBB2JXR3W4NLHB',
      phone: '+6591234567',
      bankAccount: 'DBS-1111222233',
      status: 'registered' as const,
    },
  ];
  ...
  for (const w of run1Workers) {
    const [disb] = await db
      ...
      .values({
        ...
        claimableBalanceId:
          w.status === 'new'
            ? `00000000${Math.random().toString(36).slice(2).toUpperCase().padEnd(56, '0')}`
            : null,
    ...
    await db.insert(schema.sdpEvents).values([
      {
        ...
        eventType: 'disbursement.claimed',
        payload: { status: 'claimed' },
        createdAt: new Date('2025-04-25T09:00:30Z'),
      },
      {
        disbursementId: disb.id,
        eventType: 'disbursement.completed',
        payload: { status: 'completed' },
        createdAt: new Date('2025-04-25T09:01:00Z'),
      },
    ]);
  }
